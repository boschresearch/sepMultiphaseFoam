...
Type Foam::advection::isoAdvection_wispCorrParallel::faceValue_wispCorrParallel
(
...
template<typename Type>
void Foam::advection::isoAdvection_wispCorrParallel::setFaceValue
(
...
void Foam::advection::isoAdvection_wispCorrParallel::limitFluxes
(
    const SpType& Sp,
    const SuType& Su
)
{
    ...
    DebugInfo
        << "isoAdvection_wispCorrParallel: Before conservative bounding: min(alpha) = "
        ...
        if (maxAlphaMinus1 > aTol || minAlpha < -aTol) // Note: tolerances
        {
            ...
            forAll(correctedFaces, fi)
            {
                ...
                if (alreadyUpdated.insert(facei))
                {
                    ...
                    alpha1_[own] +=
                        -faceValue_wispCorrParallel(dVfcorrectionValues, facei)/mesh_.V()[own];
                    if (mesh_.isInternalFace(facei))
                    {
                        const label nei = neighbour[facei];
                        alpha1_[nei] -=
                            -faceValue_wispCorrParallel(dVfcorrectionValues, facei)/mesh_.V()[nei];
                    }

                    // Change to treat boundaries consistently
                    scalar corrVf =
                        faceValue_wispCorrParallel(dVf_, facei)
                      + faceValue_wispCorrParallel(dVfcorrectionValues, facei);
...
template<class SpType, class SuType>
void Foam::advection::isoAdvection_wispCorrParallel::boundFlux
(
        ...
        if (alpha1_[celli] < -aTol || alpha1_[celli] > 1 + aTol)
        {
            ...
            for (label iter=0; iter<10; iter++)
            {
                ...
                forAll(downwindFaces, fi)
                {
                    ...
                    const scalar phif = faceValue_wispCorrParallel(phi_, facei);
...
//                    downwind faces
                ...
                forAll(facesToPassFluidThrough, fi)
                {
                    ...
                    scalar dVff = faceValue_wispCorrParallel(dVfcorrectionValues, facei);
...
template<class SpType, class SuType>
void Foam::advection::isoAdvection_wispCorrParallel::advect(const SpType& Sp, const SuType& Su)
{
    addProfilingInFunction(geometricVoF);
    DebugInFunction << endl;
    ...
    // Do the isoAdvection_wispCorrParallel on surface cells
    timeIntegratedFlux();
    ...
    Info<< "isoAdvection_wispCorrParallel: After conservative bounding: min(alpha) = "
        << minAlpha << ", max(alpha) = 1 + " << maxAlphaMinus1 << endl;
    ...
    DebugInfo
        << "isoAdvection_wispCorrParallel: time consumption = "
