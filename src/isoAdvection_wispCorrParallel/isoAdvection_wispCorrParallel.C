...
#include "isoAdvection_wispCorrParallel.H"
#include "volFields.H"
...
namespace advection
{
    defineTypeNameAndDebug(isoAdvection_wispCorrParallel, 0);
    addToRunTimeSelectionTable(advectionSchemes,isoAdvection_wispCorrParallel, components);
}
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::advection::isoAdvection_wispCorrParallel::isoAdvection_wispCorrParallel
(
...
void Foam::advection::isoAdvection_wispCorrParallel::setProcessorPatches()
{
...
void Foam::advection::isoAdvection_wispCorrParallel::extendMarkedCells
(
    bitSet& markedCell
) const
{
...
void Foam::advection::isoAdvection_wispCorrParallel::timeIntegratedFlux()
{
    // Get time step
...
void Foam::advection::isoAdvection_wispCorrParallel::setDownwindFaces
(
    const label celli,
...
) const
{
    ...
    forAll(c, fi)
    {
        ...
        const scalar phi = faceValue_wispCorrParallel(phi_, facei);
...
Foam::scalar Foam::advection::isoAdvection_wispCorrParallel::netFlux
(
...
) const
{
    ...
    {
        const label facei = c[fi];
        const scalar dVff = faceValue_wispCorrParallel(dVf, facei);
...
Foam::DynamicList<Foam::label>  Foam::advection::isoAdvection_wispCorrParallel::syncProcPatches
(
...
void Foam::advection::isoAdvection_wispCorrParallel::checkIfOnProcPatch(const label facei)
{
    if (!mesh_.isInternalFace(facei))
    {
...
void Foam::advection::isoAdvection_wispCorrParallel::applyBruteForceBounding()
{
    addProfilingInFunction(geometricVoF);
    ...
    // New addition - wisp correction (LN)
                ...
                if (alpha1_[cellI] > (1.0 - wispTol))
                    {
                        ...
                        forAll(neighbour,i)
                        {
                            if (alpha1_[i] < snapAlphaTol ) // if cell has empty neighbor --> no wisp
                            {
								isWisp_[cellI] = false;
                            }   
                        }
    	...
    	{   
    	    ...
    	    if (isA<processorFvPatch>(alphaFvPatch))
    	    {
    	        ...
    	        forAll(alphaFvPatchField,faceI)
    	        {
    	            ...
    	            {
    	                // But the adjacent cell across the process boundary is empty
    	                if (alphaNei[faceI] < snapAlphaTol)
						{
